...
$(document).ready(function () {
	/* CHOP Text */
	$('.chop-8').limitText({ length: 8, ellipsisText: '...' });
	$('.chop-16').limitText({ length: 16, ellipsisText: '...' });
...
*/
	...
	/* RUMBLER */
	$(
		'.rumble, .hrumble, .crumble, .strumble, .sthrumble, .mdrumble, .trumble, .prumble'
	).jrumble();
	$('.hrumble').hover(
		function () {
	...
	$('.crumble').click(function () {
		$(this).trigger('startRumble');
	}); /* start rumble when clicked */
	$('.strumble').click(function () {
		$(this).trigger('stopRumble');
	}); /* stop constant rumble when clicked */
	$('.sthrumble').hover(function () {
		$(this).trigger('stopRumble');
	}); /* stop constant rumble upon hover */
	$('.mdrumble').bind({
		mousedown: function () {
			$(this).trigger('startRumble');
		},
	...
	var demoStart = function () {
		$('.prumble').trigger('startRumble');
		setTimeout(demoStop, 300);
	};
	var demoStop = function () {
		$('.prumble').trigger('stopRumble');
	...
	var b = baffle('.baffle', { characters: '█▓▒░', speed: 150 }).reveal(
		2500
	); /* Runs animation only once, upon page load */
	var bl = baffle('.baffle-longer', {
		characters: '█▓▒░',
		speed: 150,
		duration: 3000,
	}).reveal(3000); /* Animation duration set to 3 seconds */
	var bp = baffle('.baffle-prolonged', {
		characters: '█▓▒░',
		speed: 300,
		duration: 10000,
	}).reveal(10000); /* Animation duration set to 10 seconds */
	var bf = baffle('.baffle-forever', {
		characters: '█▓▒░',
		speed: 150,
